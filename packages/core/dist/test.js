...
import { NotificationError, NotificationService, buildNotificationPreview, parseWhatsAppDecision } from "./notification.js";
...
async function run() {
    ...
    await test("RiskEngine scores permanent delete as critical", () => {
    ...
    await test("Config save/load preserves Docker upstreams and param-aware policy rules", () => {
        ...
        assert.equal(loaded.upstreamServers[0]?.transport === "docker" ? loaded.upstreamServers[0].image : "", "ghcr.io/example/filesystem-mcp:latest");
    ...
    await test("PolicyEngine overrides heuristic approvals", () => {
        ...
        const notifyRisk = policyEngine.applyToRisk("delete_email", riskEngine.score({
            toolName: "delete_email",
            payload: { id: "msg_1" },
            sessionTask: "clean inbox",
            now: new Date("2026-04-12T12:00:00Z")
        ...
        const approvedRisk = policyEngine.applyToRisk("read_email", riskEngine.score({
            toolName: "read_email",
            payload: { id: "msg_1" },
    ...
    await test("PolicyEngine matches upstream and params for specific tool calls", () => {
        ...
        assert.equal(policyEngine.evaluate({
            toolName: "write_file",
    ...
    `);
        db.prepare("INSERT INTO pending (token, tool, params, risk, decision, created, expires) VALUES (?,?,?,?,?,?,?)").run("legacy-token", "delete_email", "{\"id\":\"1\"}", 70, null, Date.now(), Date.now() + 5_000);
        db.close();
    ...
    await test("ApprovalService supports approve, deny, timeout, block, and execution-failure flows", async () => {
        const tempDir = createTempDirectory();
        ...
        const blocked = await service.executeWithApproval({
            toolName: "delete_email",
        ...
        assert.equal(blocked.status, "auto_blocked");
        channel.onSendApproval = (payload) => {
            setTimeout(() => {
                service.resolvePendingDecision(payload.request.code, "allow", "test");
            }, 25);
    ...
    await test("Notification helpers include approval codes and parse WhatsApp replies", () => {
        const engine = new RiskEngine();
        ...
        assert.match(preview, /ALLOW ABC12345/);
        assert.deepEqual(parseWhatsAppDecision("allow abc12345"), {
            decision: "allow",
            code: "ABC12345"
        });
    });
