...
import { ApprovalService, SQLiteApprovalStore, createNotificationService, loadConfig, parseWhatsAppDecision } from "@latchkey/core";
dotenv.config();
export async function startWebhookServer(options = {}) {
    ...
    app.post("/webhook/whatsapp", (req, res) => {
        ...
        const parsed = parseWhatsAppDecision(body);
        res.type("text/xml");
        if (!parsed) {
            res.send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>Reply with ALLOW &lt;CODE&gt; or DENY &lt;CODE&gt;.</Message></Response>`);
            return;
        }
        const resolved = service.resolvePendingDecision(parsed.code, parsed.decision, "whatsapp");
        if (!resolved) {
            res.send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>No pending approval found for code ${parsed.code}.</Message></Response>`);
            return;
        }
        res.send(`<?xml version="1.0" encoding="UTF-8"?><Response><Message>Action ${parsed.decision}d for ${resolved.code}.</Message></Response>`);
    });
    app.post("/webhook/slack", (req, res) => {
        try {
            ...
            res.json({ text: `Action ${decision}d for ${resolved.code}.` });
        }
        catch {
...
<html>
  ...
  <body>
    <div class="card">
      ...
      <p>Request code: <strong>${resolved.code}</strong></p>
