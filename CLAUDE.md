...
npm run test --workspace @latchkey/mcp       # builds then runs build/runtime-test.js
npm run test --workspace @latchkey/webhook   # builds then runs build/test.js
...
- `cli/mcp-discovery.ts` — **Claude Code-first MCP server discovery**: exports `discoverMcpServers()` which reads `~/.claude/settings.json` first (Claude Code), falls back to Claude Desktop config. Also exports `readMcpServersFromFile(filePath)` (used directly in tests), path helpers `getClaudeCodeSettingsPath()` / `getClaudeDesktopConfigPath()`, and removal helpers `removeServersFromClaudeCodeConfig()` / `removeServersFromClaudeDesktopConfig()`. All reads are resilient — missing file, invalid JSON, missing `mcpServers`, and malformed entries all return `[]` without crashing.
- `cli/setup.ts` (the `latchkey init` wizard): uses `discoverMcpServers()` from `mcp-discovery.ts`; shows "Claude Code" or "Claude Desktop" in prompts depending on where servers were found; removes wrapped servers from the correct source config; writes `latchkey` entry to Claude Desktop config and/or `~/.claude/settings.json`
