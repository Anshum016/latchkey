...
function extractStructuredRulesBlock(content: string): string | null {
  const commentMatch = content.match(
    /<!--\s*latchkey-rules:start\s*-->([\s\S]*?)<!--\s*latchkey-rules:end\s*-->/i
  );
...
export function parseSecurityRules(content: string): SecurityRule[] {
  ...
  if (parsed && typeof parsed === "object" && "rules" in parsed) {
    return securityRulesSchema.parse((parsed as { rules: unknown }).rules);
  }

  throw new Error("SECURITY.md rules block must be a JSON array or an object with a 'rules' array.");
}

export function loadSecurityRules(projectDir = process.cwd()): SecurityRule[] {
