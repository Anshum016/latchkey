...
export class RiskEngine {
    ...
    scoreToolBase(toolName, payload = {}) {
        const ctx = { toolName, payload };
        const rawTotal = calculateReversibility(ctx).score +
    ...
    score(ctx) {
        ...
        return {
            ...
            action: score >= BLOCK_THRESHOLD ? "block" : score >= NOTIFY_THRESHOLD ? "notify" : "approve",
            breakdown,
...
function calculateBlastRadius(ctx) {
    const serialized = JSON.stringify(ctx.payload);
...
function calculateDataSensitivity(ctx) {
    const serialized = JSON.stringify(ctx.payload);
    let score = 0;
...
function calculateIntentAlignment(ctx) {
    if (!ctx.sessionTask) {
...
function calculateTemporalAnomaly(ctx) {
...
function calculateExternalScope(ctx) {
