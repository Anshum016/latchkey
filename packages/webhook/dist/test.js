...
import { ApprovalService, NotificationService, RiskEngine, SQLiteApprovalStore } from "@latchkey/core";
...
async function run() {
    ...
    const whatsappResponse = await fetch(`${baseUrl}/webhook/whatsapp`, {
        method: "POST",
        headers: {
            "Content-Type": "application/x-www-form-urlencoded"
        },
        body: `Body=ALLOW%20${pending.code}`
    });
    assert.equal(whatsappResponse.status, 200);
    assert.equal(store.getRequest(pending.code)?.status, "approved");
    ...
    const emailResponse = await fetch(`${baseUrl}/approve?token=${encodeURIComponent(secondRequest.token)}&decision=deny`);
