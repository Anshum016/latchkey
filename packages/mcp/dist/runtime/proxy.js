...
export async function buildProxyTools(server, options) {
    const session = {
        task: "",
        startTime: Date.now(),
        callCounts: {}
    };
    server.registerTool("latchkey_set_task", {
    ...
    for (const upstream of options.config.upstreamServers) {
        try {
            ...
            const transportConfig = buildUpstreamTransportConfig(upstream, options.projectDir ?? process.cwd());
            ...
            console.error(`[Latchkey] failed to connect upstream "${upstream.name}":`, error instanceof Error ? error.message : error);
        }
    }
    return connectedUpstreams;
}
function registerUpstreamTool(server, client, upstreamName, tool, session, options) {
    ...
    const exposedToolName = shouldProtect && options.config.toolNameMode === "prefixed" ? `latchkey_${tool.name}` : tool.name;
    if (shouldProtect) {
        server.registerTool(exposedToolName, {
    ...
    server.registerTool(tool.name, {
        ...
        try {
            ...
            return createToolError(`Error executing ${tool.name}: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
async function handleProtectedToolCall(toolName, upstreamName, params, client, session, options) {
    ...
    switch (outcome.status) {
        ...
        case "auto_blocked":
            return createToolError(`Latchkey auto-blocked ${toolName}.\nRisk: ${risk.score}/100\nReason: ${risk.explanation}\nCode: ${outcome.request.code}`);
        case "denied":
        ...
        case "execution_failed":
            return createToolError(`Tool execution failed after approval.\nTool: ${toolName}\nCode: ${outcome.request.code}\nError: ${outcome.error instanceof Error ? outcome.error.message : String(outcome.error)}`);
        case "approved":
        case "pending":
